...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from the workspace-scoped history capability or configured transcript export. Don't glob across `unrelated workspaces`. That reads unrelated private chats. Walk the log against what actually happened:
