...
- Any prose surface → the **unslop** skill. Your reply is a prose surface. Write it per **Writing the reply**. Agent-facing prose also follows the **author-skill** skill.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → the bundled **deslop** skill.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the matching control skill. Use available terminal tools for CLIs/TUIs and browser tools for web UIs, or a project-provided control skill. For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
...
**Act within scope.** Proceed with authorized work. External communication and mutations require the user authorization and host permissions described in pstack-runtime.
...
**Playbook workers:** read the poteto-agent role prompt linked from pstack-runtime and pass it to the available delegation capability. Routed workflows use their own role prompts. Follow the runtime fallback when delegation is unavailable.
