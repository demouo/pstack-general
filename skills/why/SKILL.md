...
Before investigating, inspect the connectors and tools exposed by the current harness. Use only documented schemas; skip unavailable sources and report the gap.
...
- Permissions: read-only, including connector reads. Never grant writes to obtain read access.
